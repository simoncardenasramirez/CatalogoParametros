package co.edu.uco.CatalogoParametrosUcoLab.application.features.metadato.actualizarmetadato;
import co.edu.uco.CatalogoParametrosUcoLab.application.features.metadato.actualizarmetadato.usecase.domain.ActualizarMetadatoDomain;
import co.edu.uco.CatalogoParametrosUcoLab.application.usecase.validator.RuleValidator;
public interface ActualizarMetadatoRuleValidator extends RuleValidator<ActualizarMetadatoDomain> { }
