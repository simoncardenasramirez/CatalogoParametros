...
public final class ActualizarParametroDtoRequest {
    ...
    private void validateIdFuncionalidad() {
        if (TextHelper.isBlank(idFuncionalidad)) {
            throw new ParametroException("El identificador de la funcionalidad es obligatorio.");
        }
        try {
            UUID.fromString(idFuncionalidad);
        } catch (IllegalArgumentException e) {
            throw new ParametroException("El identificador de la funcionalidad no es valido. Valor recibido: " + idFuncionalidad);
        }
    }

    private void validateIdTipoParametro() {
        if (TextHelper.isBlank(idTipoParametro)) {
            throw new ParametroException("El identificador del tipo de parametro es obligatorio.");
        }
        try {
            UUID.fromString(idTipoParametro);
        } catch (IllegalArgumentException e) {
            throw new ParametroException("El identificador del tipo de parametro no es valido. Valor recibido: " + idTipoParametro);
        }
    }

    private void validateActivo() {
        if (!"true".equals(activo) && !"false".equals(activo)) {
            throw new ParametroException("El estado activo debe ser 'true' o 'false'. Valor recibido: " + activo);
        }
    }
}
