...
class SurrealDbFuncionalidadRepositoryTest {
    ...
    private final SurrealDbFuncionalidadRepository repositorio = new SurrealDbFuncionalidadRepository(cliente);
    private final ObjectMapper mapper = new ObjectMapper();

    @Test
    void debeGuardarTransaccionEscapadaCuandoNombreContieneCaracteresEspeciales() {
        var entidad = FuncionalidadEntity.create(ID, "O'Reilly\\UCO", PADRE, true, FECHA, null);
        ...
        assertAll(
                ...
                () -> assertTrue(consulta.getValue().contains("CREATE type::record('funcionalidades', '" + ID + "')")),
                () -> assertTrue(consulta.getValue().contains("O\\'Reilly\\\\UCO")),
                () -> assertTrue(consulta.getValue().contains("idModulo: '" + PADRE + "'")),
    ...
    void debeActualizarDatosCuandoEntidadExiste() {
        var entidad = FuncionalidadEntity.create(ID, "Actualizado", PADRE, false, null, FECHA);
        assertSame(entidad, repositorio.update(entidad));
        var consulta = ArgumentCaptor.forClass(String.class);
        verify(cliente).execute(consulta.capture());
        assertAll(
                () -> assertTrue(consulta.getValue().contains("UPDATE type::record('funcionalidades', '" + ID + "')")),
    ...
    void debeEliminarRegistroCorrectoCuandoRecibeIdentificador() {
        ...
        var consulta = ArgumentCaptor.forClass(String.class);
        verify(cliente).execute(consulta.capture());
        assertTrue(consulta.getValue().contains("DELETE type::record('funcionalidades', '" + ID + "');"));
    }
    ...
    void debeListarRegistrosCuandoConsultaCatalogo() {
        ...
        verify(cliente).execute("SELECT * FROM funcionalidades;");
    }
    ...
    void debeAplicarLimiteYDesplazamientoCuandoConsultaPagina() {
        ...
        verify(cliente).execute("SELECT * FROM funcionalidades LIMIT 5 START 10;");
    }
    ...
    void debeDetectarExistenciaCuandoNombreTieneComilla() {
        ...
        verify(cliente).execute("SELECT id FROM funcionalidades WHERE nombre = 'O\\'Reilly' LIMIT 1;");
    }
    ...
    void debeUsarUuidPredeterminadoCuandoIdentificadorNoEsValido(final String identificador) {
        ...
        registro.put("idModulo", identificador);
        responder(registro);
        var entidad = repositorio.findById(ID).orElseThrow();
        assertEquals(new UUID(0, 0), entidad.getId());
        assertEquals(new UUID(0, 0), entidad.getIdModulo());
    }
    ...
    private ObjectNode registro() {
        var registro = mapper.createObjectNode()
                .put("id", "funcionalidades:`" + ID + "`")
                .put("nombre", "Catalogo")
                .put("fechaInicio", "d'2026-01-02T10:30Z'")
                .putNull("fechaFinal");
        registro.put("idModulo", PADRE.toString()).put("activo", true);
        return registro;
    }
