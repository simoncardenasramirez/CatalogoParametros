...
class SurrealDbModuloRepositoryTest {
    ...
    private final SurrealDbModuloRepository repositorio = new SurrealDbModuloRepository(cliente);
    private final ObjectMapper mapper = new ObjectMapper();

    @Test
    void debeGuardarTransaccionEscapadaCuandoNombreContieneCaracteresEspeciales() {
        var entidad = ModuloEntity.create(ID, "O'Reilly\\UCO", PADRE, true, FECHA, null);
        ...
        assertAll(
                ...
                () -> assertTrue(consulta.getValue().contains("CREATE type::record('modulos', '" + ID + "')")),
    ...
    void debeActualizarDatosCuandoEntidadExiste() {
        var entidad = ModuloEntity.create(ID, "Actualizado", PADRE, false, null, FECHA);
        assertSame(entidad, repositorio.update(entidad));
        var consulta = ArgumentCaptor.forClass(String.class);
        verify(cliente).execute(consulta.capture());
        assertAll(
                () -> assertTrue(consulta.getValue().contains("UPDATE type::record('modulos', '" + ID + "')")),
    ...
    void debeEliminarRegistroCorrectoCuandoRecibeIdentificador() {
        ...
        var consulta = ArgumentCaptor.forClass(String.class);
        verify(cliente).execute(consulta.capture());
        assertTrue(consulta.getValue().contains("DELETE type::record('modulos', '" + ID + "');"));
    }
    ...
    void debeRetornarAusenciaCuandoNoHayRegistros(final String respuesta) {
        ...
        assertFalse(repositorio.existsByIdAplicacion(PADRE));
    }
    ...
    void debeListarRegistrosCuandoConsultaCatalogo() {
        ...
        verify(cliente).execute("SELECT * FROM modulos;");
    }
    ...
    void debeAplicarLimiteYDesplazamientoCuandoConsultaPagina() {
        ...
        verify(cliente).execute("SELECT * FROM modulos LIMIT 5 START 10;");
    }
    ...
    void debeDetectarExistenciaCuandoNombreTieneComilla() {
        ...
        verify(cliente).execute("SELECT id FROM modulos WHERE nombre = 'O\\'Reilly' LIMIT 1;");
    }

    @Test
    void debeDetectarRelacionCuandoExistenRegistrosDelPadre() {
        responder(registro());
        assertTrue(repositorio.existsByIdAplicacion(PADRE));
        verify(cliente).execute("SELECT id FROM modulos WHERE idAplicacion = '" + PADRE + "' LIMIT 1;");
    }
    ...
    void debeUsarUuidPredeterminadoCuandoIdentificadorNoEsValido(final String identificador) {
        ...
        registro.put("idAplicacion", identificador);
        responder(registro);
        var entidad = repositorio.findById(ID).orElseThrow();
        assertEquals(new UUID(0, 0), entidad.getId());
        assertEquals(new UUID(0, 0), entidad.getIdAplicacion());
    }
